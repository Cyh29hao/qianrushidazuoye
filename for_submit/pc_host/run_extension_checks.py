...
def execute_host_only_checks(
    ...
    _raise_if_cancelled(cancel_event)
    results: list[CheckResult] = []
    abbreviated_date = abbreviate_protocol_command("*SET:DATE YEAR MONTH DATE 2026 06 09")
    abbreviated_key_format = abbreviate_protocol_command("*SET:KEY FORMAT")
    left_frame = visible_text_to_frame("12.30.45", "LEFT")
    right_frame = visible_text_to_frame("12.30.45", "RIGHT")
    checks = [
        ("HOST 配置持久化", "OK", ""),
        ("HOST 本地模式行为", "OK", ""),
        ...
        (
            "协议缩写 DATE",
            "OK" if abbreviated_date == "*SET:DATE YEAR MONTH DATE 2026 06 09" else "FAIL",
            f"缩写结果: {abbreviated_date}",
        ),
        (
            "协议缩写 KEY FORMAT",
            "OK" if abbreviated_key_format == "*SET:KEY FORMAT" else "FAIL",
            f"缩写结果: {abbreviated_key_format}",
        ),
        (
            "FAQ v1.3 LEFT 显示帧",
            "OK" if left_frame == ("123045  ", 0x0A) else "FAIL",
            f"12.30.45 -> {left_frame[0].replace(' ', '_')} {left_frame[1]:02X}",
        ),
        (
            "FAQ v1.3 RIGHT 显示帧",
            "OK" if right_frame == ("  540321", 0x28) else "FAIL",
            f"12.30.45 RIGHT -> {right_frame[0].replace(' ', '_')} {right_frame[1]:02X}",
        ),
    ]
    if full:
        checks.extend(
