...
8 位 LED & D1 心跳、D2 闹钟、D3 编辑、D4 串口 RX、D5 串口 TX、D6 夜间、D7 RIGHT、D8 NTP 同步。\\
...
\subsection{LED 位义}
8 位 LED 在本项目中不是装饰，而是系统状态的一部分。D1 表示运行心跳，D2 表示板载单次闹钟启用或响铃闪烁，D3 表示编辑状态，D4/D5 表示串口 RX/TX 活动，D6 表示夜间模式，D7 表示 RIGHT 显示方向，D8 表示 NTP 同步状态。天气短显或 LED 掩码接管期间，LED 会临时覆盖整组状态位，结束后恢复系统位义。PC 调试页中的 \code{*SET:LED XX} 可接管 LED 约 10 秒，用于验收逐位点亮和调试；\code{*SET:LED 00} 可立即恢复。
...
\subsection{构建和运行验证}
2026-06-17 已按 FAQ v1.3 重新编译并烧录 MCU 工程，Keil 日志为 0 Error、0 Warning、Flash Verify OK，随后 COM5 严格回归全部通过。PC 端主要 Python 模块通过 \code{py_compile}，host-only 自动检查、历史 COM5 quick/full 联合测试和老师协议回归均已记录在 \code{docs/} 和 \code{PROJECT_CONTEXT.md} 中。打包版 v2.2 exe/zip 已重新生成，release 目录确认不包含会覆盖用户配置的 \code{config.json}、\code{runtime_state.json}、\code{schedules.json} 或日志目录。
...
验证项 & 结果\\
\midrule
MCU Keil5 构建 & FAQ v1.3 固件已烧录；Flash Verify OK，COM5 严格回归 PASS=27 FAIL=0。\\
