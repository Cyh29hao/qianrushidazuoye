...
#define MANUAL_LED_SHOW_MS         10000UL
#define WEATHER_SHOW_MS            5000UL
#define SYNC_DRIFT_MS              86400000UL
                                  ...
                                  uint8_t startIndex, DisplayFrame *frame)
{
    ...
    if ((g_displayFormat == FORMAT_RIGHT) && (count < DISPLAY_WIDTH)) {
        offset = (uint8_t)(DISPLAY_WIDTH - count);
    }

    for (index = 0U; index < DISPLAY_WIDTH; index++) {
        uint8_t cellIndex = startIndex + index;
        uint8_t targetIndex = offset + index;
        if (cellIndex >= count) {
            break;
        }
        if (targetIndex >= DISPLAY_WIDTH) {
            break;
        }
        frame->chars[targetIndex] = cells[cellIndex].ch;
        if (cells[cellIndex].dp != 0U) {
            frame->dp_mask |= (uint8_t)(1U << targetIndex);
        }
    }
}
...
static void ApplyEditBlink(DisplayFrame *frame)
{
    ...
    if (g_displayFormat == FORMAT_RIGHT) {
        ...
        start = (uint8_t)(mappedStart + offset);
        end = (uint8_t)(mappedEnd + offset);
    }
...
static uint8_t BuildSystemLedByte(void)
{
    ...
    if (g_lastSyncMs != 0UL) {
        uint32_t sinceSync = g_millis - g_lastSyncMs;
        if ((sinceSync < SYNC_DRIFT_MS) || (g_heartbeatBit != 0U)) {
            result |= LED_SYNC_STATUS;
        }
    }
    return result;
}
...
static void HandleSetLed(const char *params)
{
    ...
    g_manualLedUntilMs = (value == 0U) ? 0UL : (g_millis + MANUAL_LED_SHOW_MS);
    RefreshDisplayAndLeds(true);
    UART_ReplyOk(NULL);
}
