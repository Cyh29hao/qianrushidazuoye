...
def protocol_token_matches(token: str, canonical: str, min_len: int) -> bool:
    token = token.strip().upper()
    canonical = canonical.upper()
    return min_len <= len(token) <= len(canonical) and canonical.startswith(token)


def canonical_protocol_token(
    token: str,
    specs: dict[str, tuple[str, int]],
) -> str | None:
    for result, (canonical, min_len) in specs.items():
        if protocol_token_matches(token, canonical, min_len):
            return result
    return None
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _oriented_text_to_frame(self, oriented: str) -> tuple[str, int]:
        return oriented_text_to_frame(
            oriented,
            right_align=self.runtime_state.format == "RIGHT",
        )
    ...
    def _parse_protocol_field_ints(
        ...
        field_specs = {
            field: LOCAL_FIELD_SPECS.get(field, (field, len(field)))
            for field in fields
        }
        payload = parts[1:]
        ...
        for index in range(pair_count):
            ...
            field = canonical_protocol_token(field, field_specs) or ""
    ...
    def _canonicalize_local_protocol_command(self, command: str) -> str:
        upper = command.strip().upper()
        if upper.startswith("*GET:"):
            query = upper.removeprefix("*GET:").strip()
            canonical = canonical_protocol_token(query, LOCAL_GET_COMMAND_SPECS)
            return f"*GET:{canonical}" if canonical else upper
        if upper.startswith("*SET:"):
            payload = upper.removeprefix("*SET:").strip()
            if not payload:
                return upper
            parts = payload.split(maxsplit=1)
            canonical = canonical_protocol_token(parts[0], LOCAL_SET_COMMAND_SPECS)
            if canonical is None:
                return upper
            rest = f" {parts[1]}" if len(parts) > 1 else ""
            return f"*SET:{canonical}{rest}"
        return upper

    def _validate_local_protocol_command(self, command: str) -> str | None:
        upper = self._canonicalize_local_protocol_command(command)
        if not upper:
            return "SYNTAX"
        ...
        if upper.startswith("*SET:RING "):
            ...
            return None if canonical_protocol_token(value, LOCAL_RING_SPECS) in RING_TYPES else "PARAM"
    ...
    def _local_protocol_reply(self, command: str, *, query_already_handled: bool = False) -> None:
        ...
        upper = self._canonicalize_local_protocol_command(cleaned)
    ...
    def _build_debug_hardware_group(self, parent: QtWidgets.QWidget) -> QtWidgets.QGroupBox:
        ...
        hint = QtWidgets.QLabel(
            "蜂鸣用于快速听测板载蜂鸣器；LED 掩码输入 00-FF，发送 *SET:LED XX 后会接管 8 位 LED 约 10 秒，00 立即恢复系统状态。",
            group,
        )
        ...
        led_map_hint = QtWidgets.QLabel(
            "LED 位义：D1心跳  D2闹钟  D3编辑  D4串口RX  D5串口TX  D6夜间  D7RIGHT  D8 NTP同步；天气短显/LED掩码会临时覆盖整组 LED。",
            group,
        )
    ...
    def _abbreviate_protocol_command(self, command: str) -> str:
        return abbreviate_protocol_command(command)
    ...
    def _apply_local_command(self, command: str) -> None:
        ...
        if upper.startswith("*SET:DATE "):
            ...
            ring = canonical_protocol_token(upper.removeprefix("*SET:RING ").strip(), LOCAL_RING_SPECS)
            action = f"铃声预览 -> {ring or upper.removeprefix('*SET:RING ').strip()}"
