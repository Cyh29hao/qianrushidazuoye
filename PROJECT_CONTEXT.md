...
- 主页 Matplotlib 图表新增第四个筛选“日志统计”，按过去 24 小时 `logs/events.jsonl` 统计板端/显示、NTP/天气、闹钟日程、昼夜模式、自动测试、系统设置、错误警告等操作数量。
- 自动测试脚本新增 `[STEP 当前/总数]` 进度输出；PC 左侧测试框显示 `[当前/总数] 项目名`，快速串口约 11 项，全面串口约 21 项，host-only full 13 项。
- 打包版默认用户配置目录改为 `%APPDATA%\SmartClockHost-v2.2`；首次启动会把旧 release 文件夹里的配置、日程、运行状态和事件日志复制到 AppData，但不会覆盖 AppData 里已有的新数据。烟测/打包仍使用临时 `SMARTCLOCK_PROFILE_DIR`，不能污染用户配置。
- 已验证：Python 语法检查通过，host-only full 通过；源码版真实 Qt 探针在 1600x900 下连续切换四个图表筛选、反复打开/关闭页面菜单，图表错误为空，右侧数字孪生保持 420px 未被挤掉。
