...
## 0. 最新补记：Matplotlib E4 已补齐

2026-06-11 下午又补了最后一笔：主页数据看板新增“卡片看板 / Matplotlib 图表”切换。Matplotlib 图表页包含 `今日时间轴`、`提醒分布`、`系统状态` 三个筛选，已在源码版真实 Qt 窗口下截图验证 DAY/NIGHT 可读、右侧数字孪生没有被挤掉。E4 文档口径现在应按“Matplotlib 图表看板已完成”讲，不再说“轻量替代”。

新增/更新的关键文件：`pc_host/app.py`、`pc_host/requirements.txt`、`docs/screenshots/home-matplotlib-*.png`、README 和验收/答辩文档。源码版首次运行需要重新安装 requirements；打包版 v2.2 exe/zip 已重新生成并完成 8 秒临时 profile 烟测。

## 1. 当前整体状态

这个项目已经从基础数字钟收口到可演示的 v2.2：S800/TM4C1294 板端 + PyQt5 PC 上位机，支持串口同步、数字孪生、NTP 对时、天气短显、全球时区、自动昼夜、Matplotlib 数据可视化、板载单次闹钟、PC 多日程提醒、滚动消息、蜂鸣/铃声、LED 掩码、快速/全面两套自动测试和高并发防卡死保护。
